...
import { formatCurrency, formatCurrencyCents, formatMonths, formatPercent, formatSignedBps, formatSignedCurrency } from '../../../lib/format'
...
export function RefinanceCard({ analysis }: { analysis: AnalysisResponse }) {
  ...
  const verdict = worthwhile
    ...
    : `Your rate is ${formatSignedBps(-refinance.rate_advantage_bps)} below today's ${formatPercent(market.mortgage_rate_30y)} market rate. Refinancing would not lower it.`
  return (
    <Card
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-5 md:grid-cols-4">
        ...
        <Metric label="Monthly change" value={formatSignedCurrency(-refinance.monthly_savings)} hint="Negative is savings" tone={toneForSign(refinance.monthly_savings)} />
        <Metric
          label="Break-even"
